...
<?	
	...
	$numberOfGames;
	$opponent_uid;
	$sql_temp = "SELECT users_info FROM games WHERE game_id =".$_GET['gid'];
	$res_temp = mysql_fetch_assoc(mysql_query($sql_temp));
	$temp_explode = explode('|', $res_temp['users_info']);
	foreach ($temp_explode as $key => $value) {
		$tp = explode(",", $value);
		if($tp[0]!=$user) {
			$opponent_uid = $tp[0];	
			$gamescount = mysql_fetch_assoc(mysql_query("SELECT (won+lost+drawn) AS completed FROM users_stats WHERE email='".$opponent_uid."'"));
			$numberOfGames = $gamescount['completed'];
		}
	}


	if(($lang == "EN") || ($lang == 'IT')) {
            $swf = "Lexulous_Eng_1b.swf";
    } else {
          //  $swf = "lexulousv3$lang". "c.swf";
       $swf = "Lexulous_Fr_1c.swf";
    }
?>
<script type="text/javascript">	
	var _accessToken = '<?=$accessToken?>';
	var _key = "<?=$facebook->getAccessToken()?>"
	var user_id = "<?=$user?>";	
	var opponent_user_id = "<?=$opponent_uid;?>";
	var pop = false;	
	var APP_PATH = '<?=APP_PATH?>';
</script>

<script type="text/javascript" src="<?=CDN_URL_1?>js/notify.js"></script>

<script>
	<? 
		if(isset($_GET['pop'])){
			echo "update_pending('".$user."',".$_GET['gid'].");";
			if($_GET['pop']=='true') {
				echo "create_receive_notificationPopup('$user','elex_gameContainer');";
				?>
					pop=true;
				<?				
			}
...
?>
	...
	<? //Notify Plugin ?>
	<script type="text/javascript">
		setPopupBlockTable('<?=$user?>','<?=$opponent_uid?>',"<?=$_GET['gid']?>");
	</script>
...
?>	
		...
		<?if($hide_advert_option=='n'){?>
			<div style="width:300px;float: left;margin-top: 4px;">
				...
				<script type='text/javascript'><!--//<![CDATA[
				   var m3_u = (location.protocol=='https:'?'https://aws.rjs.in/adserver/www/delivery/ajs.php':'http://aws.rjs.in/adserver/www/delivery/ajs.php');
				...
				//]]>--></script><noscript><a href='http://aws.rjs.in/adserver/www/delivery/ck.php?n=afae948e&amp;cb=INSERT_RANDOM_NUMBER_HERE' target='_blank'><img src='http://aws.rjs.in/adserver/www/delivery/avw.php?zoneid=5&amp;cb=INSERT_RANDOM_NUMBER_HERE&amp;n=afae948e' border='0' alt='' /></a></noscript>
	...
	<div style="margin: 0 auto; width: 666px;">
			<div class="notifi_row" id="notif_row">
...
<? //Notify Plugin 
	// if(isset($_GET['pop'])){
	// 	echo "<script>update_pending('".$user."',".$_GET['gid'].");</script>";
	// 	if($_GET['pop']=='true') {
	// 		echo "<script>create_receive_notificationPopup('$user','elex_gameContainer')</script>";
	// 		echo "<script>show_receive_notification_pop()</script>";
	// 	}
	// }
?>

<script type="text/javascript">
	afterTurnPopup("<?=$user?>","<?=$_GET['gid']?>",'<?=$opponent_uid;?>',<?=$numberOfGames?>,'elex_gameContainer',"<?=$myfb_name?>");
</script>
