...
include_once("notify/notify_redirect.php"); //Notify Plugin 
session_start();
...
if ($inst == 1){
	...
	try {
		...
		$query_notification = "insert into notification_logs set userid='".$user."', install_datetime=now(), datetime='0000-00-00 00:00:00', messagetext='', clicked='n', friend_list='".addcslashes($friend_list,"'")."'";
...
//-----#1774---------
 ...
 if ($user) {
 	...
 	//Notify Plugin  	
 	if($_REQUEST['ref'] == 'notif' && isset($_REQUEST['notif_t'])){ 		
 		$game_id;
 		if($_REQUEST['notif_t']=='app_request'){
 			$data = $facebook->api('/me/apprequests/');  		
 			$game_id = getGameIDfromMesageData($data);
 		} else if($_REQUEST['notif_t']=='app_notification'){
 			$game_id=$_REQUEST['gid'];
 		}

 		$fetch_game_det = "SELECT * FROM games WHERE game_id =".$game_id;
 		$fetch_game_det_res = mysql_fetch_assoc(mysql_query($fetch_game_det));

 		$pid=0;
 		$password;
		$gametype=$fetch_game_det_res['game_type'];
		$lang=$fetch_game_det_res['language'];

 		$temp_explode = explode('|', $fetch_game_det_res['users_info']);
 		...
 		$redirectURL = FB_APP_PATH . "?action=viewboard&gid={$game_id}&pid={$pid}&password={$password}&gametype={$gametype}&lang={$lang}"; 		
 		if($pid==0)
 			echo "<script type='text/javascript'>top.location.href = '".FB_APP_PATH."';</script>";
 		else
 			redirectToOnNotificationClick($user,$redirectURL);
 	}
 	//----------------
...
/*if($action == "mysettings"){
	echo $que="SELECT newboard FROM settings WHERE user_id='$user'";
