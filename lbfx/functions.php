...
function get_stats_dynamodb($email){
	$Getfile = new FileIOModel();
	$result_array = $Getfile->getFile($email, "userstats");
	return $result_array;
}

function update_stats_dynamodb($email,$update_str){
	$Getfile = new FileIOModel();
	$update_str = json_encode($update_str);
	$Getfile->writeFile($email, "userstats", $update_str);
}
...
function startnewgame($user, $with, $dictionary, $game_type, $opponentname, $mobile='n') {
	...
	$default_tiles_arr=array (
							'tile_a'=>'9','tile_b'=>'2','tile_c'=>'2','tile_d'=>'4','tile_e'=>'12','tile_f'=>'2','tile_g'=>'3','tile_h'=>'2','tile_i'=>'9','tile_j'=>'1','tile_k'=>'1','tile_l'=>'4','tile_m'=>'2','tile_n'=>'6','tile_o'=>'8','tile_p'=>'2','tile_q'=>'1','tile_r'=>'6','tile_s'=>'4','tile_t'=>'6','tile_u'=>'4','tile_v'=>'2','tile_w'=>'2','tile_x'=>'1','tile_y'=>'2','tile_z'=>'1',
							'tile_blank'=>'2',
							'p1hand'=>'NULL','p2hand'=>'NULL','p3hand'=>'NULL','p4hand'=>'NULL',
							'p1prev_rack'=>'','p2prev_rack'=>'','p3prev_rack'=>'','p4prev_rack'=>'',
							'p1score'=>'0','p2score'=>'0','p3score'=>'0','p4score'=>'0'
					);
	$default_tiles_str=json_encode($default_tiles_arr);
	$Getfile = new FileIOModel();
 	$Getfile->writeFile($id, "tiles", $default_tiles_str);
	
	//$Getfile->writeEmptyFile($id, "tiles");
    ...
    for ($i = 0; $i < $no_play; $i++)
    {
        ...
        $user_stats_json=get_stats_dynamodb($email);
        $user_stats_arr = json_decode($user_stats_json[0],true);
        $user_stats_arr['played']=$user_stats_arr['played']+1;
        update_stats_dynamodb($email,$user_stats_arr);
        
        if(mysql_affected_rows() <= 0) {
        	mysql_query("INSERT INTO `users_stats` set email = '$email', played = 1");
        	        	
        	$user_stats_arr=array();        	
        	$user_stats_arr['id']=mysql_insert_id();
        	$user_stats_arr['email']=$email;
        	$user_stats_arr['rating']=1200;
        	$user_stats_arr['played']=1;
        	$user_stats_arr['won']=0;
        	$user_stats_arr['lost']=0;
        	$user_stats_arr['drawn']=0;
        	$user_stats_arr['bestrating']=0;
        	$user_stats_arr['continiouswon']=0;
        	$user_stats_arr['bestrating_date']='0000-00-00 00:00:00';
        	update_stats_dynamodb($email,$user_stats_arr);       	
        }
    }
    ...
    foreach ($giventiles as $key => $value)
    {
        if ($key == "*")
            $key = "blank";
        $sql_string .= "`tile_" . strtolower($key) . "`=tile_" . strtolower($key) . "-" .$value . ",";
    ...
    //echo $s1="UPDATE ".$tmp_games_fre_tiles_table." SET ".$tmpTilesStr.$sql_string." WHERE gameid=".$id;
    
	//mysql_query("insert into test set data='".$s1."'") or die('dffddfdfdfdfdfgdfgdfgdgdf');
...
function load_settings($uid, $getfromcache = false) {
    ...
    if($_COOKIE['html5board']){
      ...
      $qres2 = mysql_query($que);
		...
		$user_stats_arr=array();
		$user_stats_arr['id']=mysql_insert_id();
		$user_stats_arr['email']=$uid;
		$user_stats_arr['rating']=1200;
		$user_stats_arr['played']=0;
		$user_stats_arr['won']=0;
		$user_stats_arr['lost']=0;
		$user_stats_arr['drawn']=0;
		$user_stats_arr['bestrating']=0;
		$user_stats_arr['continiouswon']=0;
		$user_stats_arr['bestrating_date']='0000-00-00 00:00:00';
		update_stats_dynamodb($uid,$user_stats_arr);
...
function total_finishedGamesDetails_monthwise($email_id,$month,$year){
    ...
    $sql = "SELECT  finishedon, winner, language, player_id, {$gamestable}.game_id, password, game_type  FROM {$userstable}, {$gamestable} WHERE {$userstable}.game_id = {$gamestable}.game_id and {$userstable}.email='$email_id' order by {$gamestable}.startedon desc";
	//$sql = "SELECT  finishedon, winner, language, player_id, games_over.game_id, password, game_type  FROM users_over, games_over WHERE users_over.game_id = games_over.game_id and users_over.email='$email_id' order by games_over.finishedon desc";
	$result = mysql_query($sql);
	$cnt = mysql_num_rows($result);
	$returnArr = array();
